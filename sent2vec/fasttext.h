...
namespace fasttext {
    ...
    public:
		...
		void find_knn_by_query(const std::string& content, hnswlib::HierarchicalNSW<float>* appr_alg, int k, std::vector<std::string> sents);

		void add_points(hnswlib::HierarchicalNSW<float>* appr_alg);

		void print_sentence_vectors(const std::string path, const std::string save_path);
